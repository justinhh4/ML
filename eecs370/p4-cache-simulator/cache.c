...
int cache_access(int addr, int write_flag, int write_data)
{
    ...
    int set_index = (addr / cache.blockSize) % cache.numSets;
    int tag = addr / cache.blockSize / cache.numSets;
    // fetch/lw, does not use write_data
    if (write_flag == 0){ 
        // check if its already in cache
        for (int i = set_index * cache.blocksPerSet; i < (set_index + 1) * cache.blocksPerSet; i++){
            if (cache.blocks[i].valid == 1 && tag == cache.blocks[i].tag){
                //update lru
                for (int j = set_index * cache.blocksPerSet; j < (set_index + 1) * cache.blocksPerSet; j++){
        ...
        // pull from memory when theres open cache spot
        int empty_spot = 0; 
        for (int i = set_index * cache.blocksPerSet; i < (set_index + 1) * cache.blocksPerSet; i++){
            if (cache.blocks[i].valid == 0){
                ...
                cache.blocks[i].dirty = 0; 
                int k = 0; 
                for (int j = addr - addr % cache.blockSize; j < addr - addr % cache.blockSize + cache.blockSize; j++){
                    cache.blocks[i].data[k] = mem_access(j, 0, 0);
                    k++; 
                }
                // update lru
                for (int j = set_index * cache.blocksPerSet; j < (set_index + 1) * cache.blocksPerSet; j++){
        ...
        // pull from memory when cache is full
        if (empty_spot == 0){
            for (int i = set_index * cache.blocksPerSet; i < (set_index + 1) * cache.blocksPerSet; i++){
                // use lru to find which to evict 
                int evict_index = 0; 
                int max_lru = 0; 
                for (int j = set_index * cache.blocksPerSet; j < (set_index + 1) * cache.blocksPerSet; j++){
                    if (cache.blocks[j].valid == 1 && cache.blocks[j].lruLabel > max_lru){
                        evict_index = j; 
                        max_lru = cache.blocks[j].lruLabel; 
                    }
                }
                // write back to memory if dirty
                if (cache.blocks[evict_index].dirty == 1){
                    int evicted_block_addr = (cache.blocks[evict_index].tag * cache.numSets + set_index) * cache.blockSize;
                    for (int j = 0; j < cache.blockSize; j++){
                        mem_access(evicted_block_addr + j, 1, cache.blocks[evict_index].data[j]);
                    }
                    printAction((cache.blocks[evict_index].tag * cache.numSets + set_index) * cache.blockSize, cache.blockSize, cacheToMemory);
                }
                else {
                    printAction((cache.blocks[evict_index].tag * cache.numSets + set_index) * cache.blockSize, cache.blockSize, cacheToNowhere);
                }
                // write memory to cache 
                int k = 0; 
                for (int j = addr - addr % cache.blockSize; j < addr - addr % cache.blockSize + cache.blockSize; j++){
                    cache.blocks[evict_index].data[k] = mem_access(j, 0, 0);
                    k++; 
                }
                // update lru
                for (int j = set_index * cache.blocksPerSet; j < (set_index + 1) * cache.blocksPerSet; j++){
                ...
                cache.blocks[evict_index].lruLabel = 1; 
                cache.blocks[evict_index].tag = tag; 
                cache.blocks[evict_index].dirty = 0; 
                printAction(addr - addr % cache.blockSize, cache.blockSize, memoryToCache);
                printAction(addr, 1, cacheToProcessor); 
                return cache.blocks[evict_index].data[block_offset];  
            }
        }
    }
    //sw
        ...
        // pull from memory when cache has open
        int empty_spot = 0; 
        for (int i = set_index * cache.blocksPerSet; i < (set_index + 1) * cache.blocksPerSet; i++){
            if (cache.blocks[i].valid == 0){
                empty_spot = 1; 
        ...
        if (empty_spot == 0){
            for (int i = set_index * cache.blocksPerSet; i < (set_index + 1) * cache.blocksPerSet; i++){
                ...
                cache.blocks[evict_index].lruLabel = 1; 
                cache.blocks[evict_index].tag = tag; 
                cache.blocks[evict_index].dirty = 1; 
                cache.blocks[evict_index].data[block_offset] = write_data;  
                printAction(addr - addr % cache.blockSize, cache.blockSize, memoryToCache); 
                printAction(addr, 1, processorToCache);  
                return 0; 
            }
