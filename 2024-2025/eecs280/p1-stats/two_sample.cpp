...
vector<double> mean_diff_sampling_distribution(vector<double> data_A, vector<double> data_B) {
  ...
  for(int i = 1; i <= 1000; i++){
    double dif = mean(bootstrap_resample(data_A, i)) - mean(bootstrap_resample(data_B, i));
    distr.push_back(dif); 
  }
  return distr; 
}
...
pair<double, double> confidence_interval(vector<double> v, double width) {
  ...
  //   return {lower, upper};
  double lower = (1-width)/2; 
  double higher = lower + width;
  return {percentile(v, lower), percentile(v, higher)};
}
