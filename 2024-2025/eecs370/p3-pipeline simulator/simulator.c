...
int main(int argc, char *argv[]) {
    ...
    while (opcode(state.MEMWB.instr) != HALT) {
        ...
        if (opcode(state.IDEX.instr) == LW && (field1(state.IDEX.instr) == field0(state.IFID.instr) || field1(state.IDEX.instr) == field1(state.IFID.instr))
            && (opcode(state.IFID.instr) == ADD || opcode(state.IFID.instr) == NOR || opcode(state.IFID.instr) == BEQ || opcode(state.IFID.instr) == SW)){
        ...
        else if (opcode(state.IDEX.instr) == LW && field1(state.IDEX.instr) == field0(state.IFID.instr) && (opcode(state.IFID.instr) == LW)){
                newState.IDEX.pcPlus1 = state.IFID.pcPlus1;
                newState.IDEX.valA = 0;
                newState.IDEX.valB = 0; 
                newState.IDEX.instr = NOOPINSTR; 
            ...
            newState.IFID.pcPlus1 = state.pc + 1; 
            newState.IDEX.pcPlus1 = state.IFID.pcPlus1;
            int valA = state.reg[field0(state.IFID.instr)];
            int valB = state.reg[field1(state.IFID.instr)];
            newState.IDEX.valA = valA;
            newState.IDEX.valB = valB; 
        ...
        // detect and forward for add and nor
        ...
        if (opcode(state.EXMEM.instr) == LW){
            ...
            if (opcode(state.EXMEM.instr) == BEQ && state.EXMEM.eq == 1){
