...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int height_impl(const Node *node) {
    if (node == nullptr) return 0; 
    else return 1 + std::max(height_impl(node->left), height_impl(node->right)); 
  }
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
    ...
    else if (less(query, node->datum)){ //go left
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    ...
    else if (less(item, node->datum)){ // go left
  ...
  static Node * min_element_impl(Node *node) {
    ...
    else return min_element_impl(node -> left); 
  }
  ...
  static T min_two_elts(const T a, const T b, Compare less){
    if (less(a,b)){
      return a; 
    }
    else return b;
  }
  static T max_two_elts(const T a, const T b, Compare less){
    if (less(a,b)) return b; 
    else return a; 
  }

  static T find_min(const Node *node, Compare less){
    if (node -> left == nullptr && node -> right == nullptr){
      return node->datum; 
    }
    else if (node->left == nullptr){
      return min_two_elts(node->datum, find_min(node->right, less), less); 
    }
    else if (node->right == nullptr){
      return min_two_elts(node->datum, find_min(node->left, less), less); 
    }
    else { 
      return min_two_elts(min_two_elts(find_min(node->left, less),
             find_min(node->right, less), less), node->datum, less);
    }
  }

  static T find_max(const Node *node, Compare less){
    if (node -> left == nullptr && node -> right == nullptr){
      return node->datum; 
    }
    else if (node->left == nullptr){
      return max_two_elts(node->datum, find_max(node->right, less), less); 
    }
    else if (node->right == nullptr){
      return max_two_elts(node->datum, find_max(node->left, less), less);
    }
    else {
      return max_two_elts(node->datum, max_two_elts(find_max(node->left, less), 
                  find_max(node->right, less), less), less); 
    }
  }
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    if (node -> left == nullptr && node -> right == nullptr){
      ...
      if (less(node->datum, find_min(node->right, less))){
        return check_sorting_invariant_impl(node->right, less); 
      }
      else return false; 
    }
    else if (node -> right == nullptr){
      if (less(find_max(node->left, less), node->datum)){
      ...
      if (less(node->datum, find_min(node->right, less)) 
                && less(find_max(node->left, less), node->datum)){
        return (check_sorting_invariant_impl(node->left, less) 
