...
AVL::Node* AVL::rotate_left(AVL::Node* node) {
    ...
    AVL::Node* moved_node = new_root->left; 
    new_root->left = node;
    node->right = moved_node; 
    node->fix_height(); 
    new_root->fix_height();
    /*
    if (node->left == nullptr && node->right == nullptr) node->height = 1;
    else if (node->left == nullptr) node->height = 1 + node->right->height; 
    else if (node->right == nullptr) node->height = 1 + node->left->height;
    else node->height = 1+max(node->right->height, node->left->height); 
    
    if (new_root->right == nullptr) new_root->height = new_root->left->height; 
    else new_root->height = 1+max(new_root->left->height, new_root->right->height); 
    */
...
AVL::Node* AVL::rotate_right(AVL::Node* node) {
    ...
    AVL::Node* moved_node = new_root->right; 
    new_root->right = node;
    ...
    if (new_root->left == nullptr) new_root->height = new_root->right->height; 
    else new_root->height = 1+max(new_root->left->height, new_root->right->height); 
    */
