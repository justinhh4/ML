...
int get_num_mem_accesses(){
 ...
 main(int argc, char **argv)
 {
    ...
    // printState(&state);
     ...
     while (state.pc < state.numMemory){
         int opcode = cache_access(state.pc, 0, 0) >> 22; 
         if (opcode == 0){ // add
             add(&state);
         }
 ...
 void lw(stateType *statePtr){
     ...
     statePtr->reg[r2] = cache_access(offsetField + statePtr->reg[r1], 0, 0); 
     statePtr->pc++; 
 }
 ...
 void sw(stateType *statePtr){
     ...
     int r2 = (code >> 16) & 0b0111;
     int offsetField = convertNum(code & 0xFFFF); 
     cache_access(offsetField + statePtr->reg[r1], 1, statePtr->reg[r2]); 
     statePtr->pc++; 
 }
