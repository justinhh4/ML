...
{
  string rank, ignore, suit;
  is >> rank >> ignore >> suit;
  Rank rank1 = string_to_rank(rank);
  Suit suit1 = string_to_suit(suit);
  card.rank = rank1;
  card.suit = suit1;
  return is;
}
...
{
  return Card_less(a,b, Card(ACE, trump), trump); 
  // can change to use other card less if repeated code
  /*
